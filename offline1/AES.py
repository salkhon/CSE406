
class AES:
    """Symmetric cryptography system. 
    """
    SBOX = (
        ,
    )

    INV_SBOX = (
        ,
    )

    def __init__(self):
        pass

    def encrypt(self, text: str, key: str) -> str:
        """Encrypt the given text, with the provided key.

        If key has length less than 128 bits, zero pad it. 
        Else if key has length greater than 128, chunk it to 128 or less. 

        Args:
            text (str): Plain text
            key (str): Cipher secret key
        Returns:
            str: Cipher
        """
        return ""

    def decrypt(self, cipher: str, key: str) -> str:
        """Decrpypt the given cipher with the given key.

        Args:
            cipher (str): Cipher text
            key (str): Key

        Returns:
            str: Plain text
        """
        return ""

    def convert_to_16_byte_keys(self, key: str) -> list[str]:
        """Convert key of any length to list of 16 bytes keys (128 bits).

        Args:
            key (str): Key of any length

        Returns:
            list[str]: List of 16 byte keys
        """

        def pad_smaller_key(key: str) -> str:
            return '\0' * (16 - len(key)) + key

        keys = []
        if len(key) < 16:
            keys.append(pad_smaller_key(key))
        elif len(key) > 16:
            keys.extend([key[i:i+16] for i in range(0, len(key) - 16, 16)])
            keys.append(
                pad_smaller_key(
                    key[(len(key) // 16) * 16:]))

        return keys

    def convert_16_byte_key_to_4_byte_words(self, key: str) -> list[str]:
        """Convert 16 bytes key (HAS TO BE 16 BYTE KEY) to a list of 4 byte words.

        Args:
            key (str): 16 byte keys

        Returns:
            list[str]: list of 4 key words, each word having 4 bytes
        """
        return [key[i:i+4] for i in range(0, len(key), 4)]

    def key_expansion(self, key: str) -> list[str]:
        """Expand 16 byte key to 44 keys each of 4 words.

        Args:
            key (str): Key string of 16 characters

        Returns:
            list[str]: List of 44 4-characted keys
        """

        def left_rotate_4_byte_word(word: str) -> str:
            """Left rotate 4 character word.

            Args:
                word (str): 4 character word

            Returns:
                str: Left rotated word
            """
            return word[1:] + word[0]

        words = self.convert_16_byte_key_to_4_byte_words(key)
        gw4 = left_rotate_4_byte_word(words[3])

        # byte substitution
        gw4 = "".join(
            [chr(AES.SBOX[ord(c)])
             for c in gw4])

        return []


if __name__ == "__main__":
    aes = AES()
    print(aes.convert_to_16_byte_keys("abcdefghijkl" * 3))
